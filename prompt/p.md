      ...
      case "fisheye":
        ...
        for (let y = 0; y < height; y++) {
          for (let x = 0; x < width; x++) {
            ...
            if (distance < radius) {
              const factor = Math.sin(((distance / radius) * Math.PI) / 2);
              ...
              const dstIndex = (y * width + x) * 4;
              processedData[dstIndex] = 0;
              processedData[dstIndex + 1] = 0;
              processedData[dstIndex + 2] = 0;
              processedData[dstIndex + 3] = 255;
            }
          }
        }
        break;
